...
class EventOrderProjectionAvailabilityAdapterTest {
  ...
  @Test
  void cleanupDeletesExpiredProjectionRows() {
    jdbcClient
        .sql(
            """
            INSERT INTO ticket_features.t_event_order_projection
                (event_id, row_number, place_number, projected_at)
            VALUES (:eventId, 1, 2, DATEADD('HOUR', -2, CURRENT_TIMESTAMP))
            """)
        .param("eventId", EVENT_ID)
        .update();
    EventOrderProjectionAvailabilityAdapter adapter =
    ...
    adapter.cleanupExpiredProjectionRows();
    ...
    Integer projectedRows =
        jdbcClient
            ...
            .query(Integer.class)
            .single();
    assertThat(projectedRows).isZero();
  }
  ...
  private void assertProjectedPlace(Integer rowNumber, Integer placeNumber) {
    Integer projectedRows =
        jdbcClient
            .sql(
                """
            ...
            .param("placeNumber", placeNumber)
            .query(Integer.class)
            .single();
    assertThat(projectedRows).isEqualTo(1);
  }
